...
export const WeddingCalculator = () => {
  ...
  const handleCalculate = () => {
    ...
    const w1 = StandardPrimbon.getWeton(d1);
    const w2 = StandardPrimbon.getWeton(d2);
    
    const dates = StandardPrimbon.findWeddingDates(w1.neptu, w2.neptu);
    setResults(dates);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center space-y-2">
        ...
        <p className="text-text-muted text-sm">
          Find auspicious dates for your wedding based on ancient Weton rules.
        </p>
      </div>
            ...
            {results.length > 0 ? (
              <ul className="space-y-2">
                {results.map((date, idx) => {
                  const w = StandardPrimbon.getWeton(date);
                  return (
                    <li key={idx} className="p-3 bg-background/40 rounded border border-border flex justify-between items-center">
                      <span className="font-bold text-primary">{date.toLocaleDateString('id-ID', {weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'})}</span>
